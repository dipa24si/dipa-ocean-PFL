...
import { Gift, LogIn, Megaphone, Coffee, ArrowRight, HelpCircle, ChevronDown, ChevronUp } from 'lucide-react';
import TestimonialCarousel from '../../Pertemuan8/components/TestimonialCarousel';
import ScrollReveal from '../../Pertemuan8/components/ScrollReveal';
    ...
    <div className="bg-[#F4F1EA] text-[#0F0E0C] font-sans antialiased selection:bg-[#A88C74] selection:text-white overflow-x-hidden min-h-screen">
      ...
      <ScrollReveal>
        <header className="max-w-7xl mx-auto px-6 py-6 flex justify-between items-center relative z-40">
          ...
          <div className="hidden lg:flex items-center gap-4 text-[11px] font-bold tracking-[0.2em] uppercase text-[#0F0E0C]/60">
            <span className="text-[#0F0E0C]/20">•</span>
            <button
              onClick={() => navigate('/menu')}
              className="hover:text-[#0F0E0C] transition-colors font-bold tracking-[0.2em]"
            >
              MENU PAGE
            </button>
            <span className="text-[#0F0E0C]/20">•</span>
          ...
          <section className="grid grid-cols-1 lg:grid-cols-12 gap-6 pt-2">
            <div className="lg:col-span-8 bg-gradient-to-br from-[#0F0E0C] via-[#1A1815] to-[#24211D] text-[#F4F1EA] rounded-[2.5rem] p-8 md:p-12 flex flex-col justify-between shadow-2xl relative overflow-hidden group min-h-[340px]">
              ...
              <div className="self-start inline-flex items-center gap-3 bg-white/5 backdrop-blur-md px-4 py-1.5 rounded-full border border-white/10 text-[11px] font-medium tracking-wide">
                ...
                <span className="tracking-wider text-[#EAD3B3]/90">Ambient Access: Unregistered</span>
              </div>
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {featuredProducts.map((product, i) => (
              <ScrollReveal key={product.id} delay={i * 100} direction="up">
                <article className="bg-white border border-[#A88C74]/15 rounded-[2rem] p-6 shadow-sm hover:shadow-xl transition-all duration-300 relative overflow-hidden group flex flex-col justify-between h-full">
                  <div>
          ...
          <ScrollReveal>
            <div className="text-center">
              <button
                onClick={() => navigate('/menu')}
                className="inline-flex items-center gap-2 text-xs font-bold tracking-wider text-[#A88C74] hover:text-[#0F0E0C] transition-colors underline underline-offset-4"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {promos.map((promo, idx) => (
              <ScrollReveal key={promo.title} delay={idx * 100}>
                <article className="bg-white border border-[#A88C74]/15 rounded-[2rem] p-6 shadow-sm hover:shadow-xl transition-all relative overflow-hidden group hover:-translate-y-0.5">
